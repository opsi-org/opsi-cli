...
def test_package_install_and_uninstall(admin_service_client: ServiceClient) -> None:
	...
	with patch("opsicli.opsiservice.get_depot_connection", mock_get_depot_connection):
		...
		###########################################
		# Uninstall opsi-client-agent with purge
		###########################################

		exit_code, _stdout, _stderr = run_cli(
			["package", "uninstall", "--purge", "opsi-client-agent", "--depots", f"{configserver_id},depot1.opsi.test"]
		)
		assert exit_code == 0
		...
		# Verify that productOnDepot is not removed for depot2.opsi.test
		pods = sorted(
			admin_service_client.productOnDepot_getObjects(productId=["opsi-client-agent"]),  # type: ignore[unresolved-attribute]
			key=lambda p: (p.productId, p.depotId),
		)
		assert len(pods) == 1
		assert pods[0].productId == "opsi-client-agent"
		assert pods[0].depotId == "depot2.opsi.test"

		# Verify that productOnClient and productPropertyState are partly removed
		# client1.opsi.test => depot1.opsi.test
		# client2.opsi.test => depot2.opsi.test
		# client3.opsi.test => <configserver>
		pocs = sorted(
			admin_service_client.productOnClient_getObjects(productId=["opsi-client-agent"]),  # type: ignore[unresolved-attribute]
			key=lambda p: (p.productId, p.clientId),
		)
		assert len(pocs) == 1
		assert pocs[0].clientId == "client2.opsi.test"
		...
		assert len(ppss) == 1
		assert ppss[0].objectId == "client2.opsi.test"
		assert ppss[0].propertyId == "allow_reboot"
		assert ppss[0].values == [True]

		###########################################
		# Uninstall opsi-client-agent from all depots with purge
		###########################################

		exit_code, _stdout, _stderr = run_cli(["package", "uninstall", "--purge", "opsi-client-agent", "--depots", "all"])
		assert exit_code == 0

		# Verify that product is removed
		products = admin_service_client.product_getObjects(id=["opsi-client-agent"])  # type: ignore[unresolved-attribute]
		assert len(products) == 0
		# Verify that productOnDepot is removed
		pods = admin_service_client.productOnDepot_getObjects(productId=["opsi-client-agent"])  # type: ignore[unresolved-attribute]
		assert len(pods) == 0
		# Verify that productOnClient is removed
		pocs = admin_service_client.productOnClient_getObjects(productId=["opsi-client-agent"])  # type: ignore[unresolved-attribute]
		assert len(pocs) == 0
		# Verify that productPropertyState is removed
		ppss = admin_service_client.productPropertyState_getObjects(  # type: ignore[unresolved-attribute]
			productId=["opsi-client-agent"], objectId=["client1.opsi.test", "client2.opsi.test", "client3.opsi.test"]
		)
		assert len(ppss) == 0

		###########################################
		# Test purge without uninstall
		###########################################
		admin_service_client.product_updateObjects(  # type: ignore[unresolved-attribute]
			[
				LocalbootProduct(
